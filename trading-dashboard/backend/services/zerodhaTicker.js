...
module.exports = function createZerodhaTickerBridge({ io, apiKey }) {
  ...
  function start(accessToken) {
    ...
    ticker.on("connect", () => {
      ...
      console.log('[ZerodhaTicker] Connected to Kite WebSocket');
    ...
    ticker.on("disconnect", () => {
      tickerConnected = false;
      console.log('[ZerodhaTicker] Disconnected from Kite WebSocket');
      io.emit("serviceStatus", getStatus());
    });

    ticker.on("error", (error) => {
      tickerConnected = false;
      console.error('[ZerodhaTicker] Error:', error);
      io.emit("serviceStatus", getStatus());
    });
